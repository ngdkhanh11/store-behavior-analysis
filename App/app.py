...
class RTSPDialog(wx.Dialog):
    def __init__(self, parent, title):
        super(RTSPDialog, self).__init__(parent, title=title, size=(350, 150))
        panel = wx.Panel(self)

        self.rtsp_label = wx.StaticText(panel, label="Enter RTSP/HTTP Link:", pos=(10, 20))
        self.rtsp_text = wx.TextCtrl(panel, pos=(140, 15), size=(150, 25))
...
class MyFrame(wx.Frame):
    def __init__(self):
        ...
        wx.StaticText(panel, label="Browse", pos=(590, 285))
        wx.StaticText(panel, label="Get Heatmap Analysis", pos=(50, 125))
        wx.StaticText(panel, label="Open Heatmap Video", pos=(213, 125))
        wx.StaticText(panel, label="Get Report Analysis", pos=(360, 125))
        wx.StaticText(panel, label="Open Excel", pos=(520, 125))
        wx.StaticText(panel, label="Open Camera", pos=(155, 220))
        wx.StaticText(panel, label="RTSP/HTTP Camera Link", pos=(275, 220))
        wx.StaticText(panel, label="Alarm", pos=(466, 220))
    ...
    def on_rtsp(self, event):
        rtsp_dialog = RTSPDialog(self, "Enter RTSP/HTTP Link")
        result = rtsp_dialog.ShowModal()

        if result == wx.ID_OK:
            rtsp_link = rtsp_dialog.rtsp_text.GetValue()
            if rtsp_link:
                print(f"Entered RTSP/HTTP Link: {rtsp_link}")
                runDetech(rtsp_link)
